...
static int msm_eeprom_get_dt_data(struct msm_eeprom_ctrl_t *e_ctrl)
{
	...
	rc = msm_camera2_get_dt_vreg_data(of_node, &power_info->cam_vreg,
					     &power_info->num_vreg);
	if (rc < 0)
		return rc;

	rc = msm_camera2_get_dt_power_setting_data(of_node,
	...
	if (gpio_array_size) {
		...
		rc = msm_camera2_get_dt_gpio_req_tbl(of_node, gconf,
		...
		rc = msm_camera2_init_gpio_pin_tbl(of_node, gconf,
			gpio_array, gpio_array_size);
		if (rc < 0) {
			pr_err("%s failed %d\n", __func__, __LINE__);
			goto ERROR4;
...
static int msm_eeprom_spi_setup(struct spi_device *spi)
{
	...
	rc = msm_camera2_power_up(power_info, e_ctrl->eeprom_device_type,
		&e_ctrl->i2c_client);
	if (rc < 0) {
		pr_err("failed rc %d\n", rc);
		goto caldata_free;
	...
	rc = msm_camera2_power_down(power_info, e_ctrl->eeprom_device_type,
		&e_ctrl->i2c_client);
	if (rc < 0) {
		pr_err("failed rc %d\n", rc);
		goto caldata_free;
...
power_down:
	msm_camera2_power_down(power_info, e_ctrl->eeprom_device_type,
		&e_ctrl->i2c_client);
caldata_free:
	kfree(e_ctrl->cal_data.mapdata);
	kfree(e_ctrl->cal_data.map);
...
static int __maybe_unused  msm_eeprom_platform_probe(struct platform_device *pdev)
{
	...
	CDBG("%s X\n", __func__);
finish:
//	spi_register_driver(&msm_eeprom2_spi_driver);
//	i2c_add_driver(&msm_eeprom2_i2c_driver);
	return rc;
...
{
	platform_driver_unregister(&msm_eeprom2_platform_driver);
	spi_unregister_driver(&msm_eeprom2_spi_driver);
	i2c_del_driver(&msm_eeprom2_i2c_driver);
}
